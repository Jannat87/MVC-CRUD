...
<?php 
	if(isset($_POST['mobile'])){
		...
		if(empty($company) OR empty($model) OR empty($details) OR empty($price)){
			...
			header('Location:index.php?page=Mobile&msg='.$msg_save);
		}else{

			$mobile_query = "INSERT INTO tb_mobile (company,model,details,price) 
...
<form action="index.php?page=Mobile" method="post">
  ...
  <div class="form-group">
    ...
    <input name="company" type="text" class="form-control" id="exampleInputEmail1" 
    value="<?php if(isset($update_data)){ echo $update_data['company']; } ?>">
  </div>
  <div class="form-group">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <?php 
        	echo '<h3>Brand Name # '.$mobile['company'].'</h3>';
        	echo '<h3>Model # '.$mobile['model'].'</h3>';
        	echo '<h3>Price # '.$mobile['price'].'.tk</h3>';
